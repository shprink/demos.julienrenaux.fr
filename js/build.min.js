...
}]),angular.module("template/timepicker/timepicker.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/timepicker/timepicker.html",'<table>\n	<tbody>\n		<tr class="text-center">\n			<td><a ng-click="incrementHours()" class="btn btn-link"><span class="glyphicon glyphicon-chevron-up"></span></a></td>\n			<td>&nbsp;</td>\n			<td><a ng-click="incrementMinutes()" class="btn btn-link"><span class="glyphicon glyphicon-chevron-up"></span></a></td>\n			<td ng-show="showMeridian"></td>\n		</tr>\n		<tr>\n			<td style="width:50px;" class="form-group" ng-class="{\'has-error\': invalidHours}">\n				<input type="text" ng-model="hours" ng-change="updateHours()" class="form-control text-center" ng-mousewheel="incrementHours()" ng-readonly="readonlyInput" maxlength="2">\n			</td>\n			<td>:</td>\n			<td style="width:50px;" class="form-group" ng-class="{\'has-error\': invalidMinutes}">\n				<input type="text" ng-model="minutes" ng-change="updateMinutes()" class="form-control text-center" ng-readonly="readonlyInput" maxlength="2">\n			</td>\n			<td ng-show="showMeridian"><button type="button" class="btn btn-default text-center" ng-click="toggleMeridian()">{{meridian}}</button></td>\n		</tr>\n		<tr class="text-center">\n			<td><a ng-click="decrementHours()" class="btn btn-link"><span class="glyphicon glyphicon-chevron-down"></span></a></td>\n			<td>&nbsp;</td>\n			<td><a ng-click="decrementMinutes()" class="btn btn-link"><span class="glyphicon glyphicon-chevron-down"></span></a></td>\n			<td ng-show="showMeridian"></td>\n		</tr>\n	</tbody>\n</table>\n')}]),angular.module("template/typeahead/typeahead-match.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/typeahead/typeahead-match.html",'<a tabindex="-1" bind-html-unsafe="match.label | typeaheadHighlight:query"></a>')}]),angular.module("template/typeahead/typeahead-popup.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/typeahead/typeahead-popup.html",'<ul class="dropdown-menu" ng-if="isOpen()" ng-style="{top: position.top+\'px\', left: position.left+\'px\'}" style="display: block;" role="listbox" aria-hidden="{{!isOpen()}}">\n    <li ng-repeat="match in matches track by $index" ng-class="{active: isActive($index) }" ng-mouseenter="selectActive($index)" ng-click="selectMatch($index)" role="option" id="{{match.id}}">\n        <div typeahead-match index="$index" match="match" query="query" template-url="templateUrl"></div>\n    </li>\n</ul>')}]),angular.module("democenter",["demoFactory","ngRoute","ngProgress","ui.bootstrap"]).run(["$rootScope","$sce","DemoService","Demo","$routeParams",function($rootScope,$sce,DemoService,Demo){$rootScope.trustSrc=function(src){return $sce.trustAsResourceUrl(src)},$rootScope.list=function(){return Demo.query(function(results){var categories={all:{name:"all",value:"",count:results.length,href:"#demos"}};angular.forEach(results,function(value){"undefined"==typeof categories[value.category]&&(categories[value.category]={name:value.category,count:0,value:value.category,href:"#demos/"+value.category}),categories[value.category].count+=1}),$rootScope.categories=categories})}(),$rootScope.$on("$routeChangeSuccess",function(scope,next){$rootScope.currentCategory="undefined"!=typeof next.params.category?next.params.category:"",DemoService.afterLoading()}),$rootScope.getFbUri=function(category,uri){return DemoService.getFbUri(category,uri)}}]),angular.module("demoFactory",["ngResource"]).factory("Demo",function($resource){return $resource("data/:demoId.json",{},{query:{method:"GET",params:{demoId:"demos"},isArray:!0,cache:!1}})}),angular.module("democenter").config(["$sceDelegateProvider",function($sceDelegateProvider){$sceDelegateProvider.resourceUrlWhitelist(["examples.julienrenaux.fr","connect.facebook.net","www.facebook.com","self"])}]),angular.module("democenter").config(["$routeProvider",function($routeProvider){$routeProvider.when("/demos",{templateUrl:"template/demo-list.html"}).when("/demos/:category",{templateUrl:"template/demo-list.html"}).when("/demos/:category/:demoId",{templateUrl:"template/demo-item.html",controller:DemoItemCtrl}).otherwise({redirectTo:"/demos"})}]),angular.module("democenter").service("DemoService",["ngProgress","$window","$location",function(ngProgress,$window,$location){this.getFbUri=function(category,uri){return"//www.facebook.com/plugins/like.php?href="+encodeURIComponent("http://demos.julienrenaux.fr/#/demos/"+category+"/"+uri)+"&amp;width=200&amp;height=30&amp;colorscheme=light&amp;layout=button_count&amp;action=like&amp;show_faces=false&amp;send=false"},this.afterLoading=function(){var currentPageId=$location.path();"undefined"!=typeof $window._gaq&&$window._gaq.push(["_trackPageview",currentPageId])}}]),DemoItemCtrl.$inject=["$scope","$location","$routeParams","Demo","ngProgress"];